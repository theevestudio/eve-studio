...
export default function EditThemePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setForm({
        ...
        audience_age: data.audience_age?.[0] ?? '',
        audience_gender: data.audience_gender ?? '',
        audience_lifestyle: data.audience_lifestyle?.join(', ') ?? '',
        audience_wants: data.audience_wants?.join(', ') ?? '',
        audience_knowledge: data.audience_knowledge ?? '',
        content_personality: data.content_personality ?? [],
        on_camera: data.on_camera ?? '',
        speaking_style: data.speaking_style ?? '',
        hook_styles: data.hook_styles ?? [],
        topics_never: data.topics_never?.join(', ') ?? '',
        content_goals: data.content_goals ?? [],
        primary_cta: data.primary_cta ?? '',
        signature_offer: data.signature_offer ?? '',
        content_pillars: data.content_pillars?.join(', ') ?? '',
        filming_location: data.filming_location?.[0] ?? '',
        visual_vibe: data.visual_vibe ?? '',
        audio_style: data.audio_style ?? '',
        brand_differentiators: data.brand_differentiators?.join(', ') ?? '',
        real_comments: data.real_comments ?? '',
  ...
  async function handleSave() {
    ...
    const { error: err } = await supabase.from('themes').update(payload).eq('id', params.id as string)
    if (err) { setError(err.message); setSaving(false) }
    else router.push(`/dashboard/themes/${params.id}`)
  }

  const inputClass = 'w-full bg-zinc-900 border border-zinc-800 text-white rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-violet-500 transition placeholder-zinc-600'
      ...
      <main className="max-w-2xl mx-auto px-6 py-10 space-y-8">
        ...
        <Section title="Client Info">
          <Field label="Client name *">
            <input value={form.client_name} onChange={e => set('client_name', e.target.value)} className={inputClass} placeholder="e.g. The E.V.E. Studio" />
          </Field>
          <Field label="Industry">
            <input value={form.industry} onChange={e => set('industry', e.target.value)} className={inputClass} placeholder="e.g. Real Estate, Fitness, Beauty" />
          </Field>
          <Field label="Sub-niche">
            <input value={form.sub_niche} onChange={e => set('sub_niche', e.target.value)} className={inputClass} placeholder="e.g. Luxury waterfront homes" />
          </Field>
          <Field label="Videos per month">
            <input value={form.videos_per_month} onChange={e => set('videos_per_month', e.target.value)} className={inputClass} placeholder="e.g. 12" />
          </Field>
        </Section>
        ...
        <Section title="Audience">
          <Field label="Age range">
            <input value={form.audience_age} onChange={e => set('audience_age', e.target.value)} className={inputClass} placeholder="e.g. 28-45" />
          </Field>
          <Field label="Gender">
            <input value={form.audience_gender} onChange={e => set('audience_gender', e.target.value)} className={inputClass} placeholder="e.g. Women, Men, All" />
          </Field>
          <Field label="Lifestyle (comma separated)">
            <input value={form.audience_lifestyle} onChange={e => set('audience_lifestyle', e.target.value)} className={inputClass} placeholder="e.g. homeowners, outdoor lovers, high income" />
          </Field>
          <Field label="What they want (comma separated)">
            <input value={form.audience_wants} onChange={e => set('audience_wants', e.target.value)} className={inputClass} placeholder="e.g. sell faster, look credible, grow following" />
          </Field>
          <Field label="Their knowledge level">
            <input value={form.audience_knowledge} onChange={e => set('audience_knowledge', e.target.value)} className={inputClass} placeholder="e.g. beginner, intermediate, expert" />
          </Field>
        </Section>
        ...
        <Section title="Content Style">
          ...
          <Field label="On camera?">
            <input value={form.on_camera} onChange={e => set('on_camera', e.target.value)} className={inputClass} placeholder="e.g. Yes — client, Yes — editor only, Voiceover only" />
          </Field>
          <Field label="Speaking style">
            <input value={form.speaking_style} onChange={e => set('speaking_style', e.target.value)} className={inputClass} placeholder="e.g. Conversational, professional, hype" />
          </Field>
          ...
          <Field label="Topics to never cover (comma separated)">
            <input value={form.topics_never} onChange={e => set('topics_never', e.target.value)} className={inputClass} placeholder="e.g. politics, competitors, pricing" />
          </Field>
        </Section>
        ...
        <Section title="Brand Strategy">
          ...
          <Field label="Primary CTA">
            <input value={form.primary_cta} onChange={e => set('primary_cta', e.target.value)} className={inputClass} placeholder="e.g. Book a call, DM us, Visit link in bio" />
          </Field>
          <Field label="Signature offer">
            <input value={form.signature_offer} onChange={e => set('signature_offer', e.target.value)} className={inputClass} placeholder="e.g. Free home valuation, 30-min consult" />
          </Field>
          <Field label="Content pillars (comma separated)">
            <input value={form.content_pillars} onChange={e => set('content_pillars', e.target.value)} className={inputClass} placeholder="e.g. education, behind the scenes, results" />
          </Field>
        </Section>

        <Section title="Visuals & Brand">
          <Field label="Filming location">
            <input value={form.filming_location} onChange={e => set('filming_location', e.target.value)} className={inputClass} placeholder="e.g. On location, studio, outdoors" />
          </Field>
          <Field label="Visual vibe">
            <input value={form.visual_vibe} onChange={e => set('visual_vibe', e.target.value)} className={inputClass} placeholder="e.g. Clean luxury, gritty authentic, bright & fun" />
          </Field>
          <Field label="Audio style">
            <input value={form.audio_style} onChange={e => set('audio_style', e.target.value)} className={inputClass} placeholder="e.g. Trending sounds, original music, silent" />
          </Field>
          <Field label="Brand differentiators (comma separated)">
            <input value={form.brand_differentiators} onChange={e => set('brand_differentiators', e.target.value)} className={inputClass} placeholder="e.g. local expert, fastest response time, award winning" />
          </Field>
        </Section>

        <Section title="Extras">
          <Field label="Real comments / questions your audience asks">
            <textarea value={form.real_comments} onChange={e => set('real_comments', e.target.value)} className={`${inputClass} h-24 resize-none`} placeholder="Paste real DMs, comments or FAQs here..." />
          </Field>
          <Field label="Common misconception in your niche">
            <input value={form.common_misconception} onChange={e => set('common_misconception', e.target.value)} className={inputClass} placeholder="e.g. People think you need a huge budget to sell fast" />
          </Field>
        </Section>
        ...
        <div className="flex gap-3 pb-10">
          ...
          <button onClick={() => router.push(`/dashboard/themes/${params.id}`)} className="text-zinc-400 hover:text-white transition px-4 py-3 text-sm">
            Cancel
          </button>
        </div>
      </main>
